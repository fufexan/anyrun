...
pub fn handler(selection: Match, state: &State) -> HandleResult {
    ...
    let cmd = command.arg("-c");

    if let Err(why) = {
        let full_args = format!("{} {}", prefix, entry.path.to_string_lossy());

        cmd.arg(full_args).spawn()
    } {
...
pub fn init(config_dir: RString) -> State {
    let config: Config = match fs::read_to_string(format!("{}/uwsm_app.ron", config_dir)) {
        Ok(content) => ron::from_str(&content).unwrap_or_else(|why| {
            eprintln!("Error parsing uwsm_app plugin config: {}", why);
            Config::default()
        }),
        Err(why) => {
            eprintln!("Error reading uwsm_app plugin config: {}", why);
            Config::default()
        }
    };
